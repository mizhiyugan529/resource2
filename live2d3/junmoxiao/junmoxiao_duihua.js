...
function allthescript() {
    ...
    var modelDir = ['junmoxiao']; // 指定需要加载的模型
    ...
    setTimeout(function () {
        ...
        $("#live2ddiv").click(function () {
                playLive2dMotion("ye", 0, 3);
            const list_click=["卖材料了，童叟无欺！","强力蛛丝收不收？","别闹...强力蛛丝收不收？"]
            if (now >= 21 || now <= 5){
...
//
    ...
    function move() {
        if (flag) {
            var list_move=["买材料吗？不买就不买，跑什么啊","我真就卖个材料，别怕"]
            if (now >= 21 || now <= 5){
...
//鼠标释放时候的函数
    ...
    (function (){
        ...
        if (now > 23 || now <= 5) {
            ...
            text = '嗨，早啊，大清早的人就是少，半天也看不见一个';
        } else if (now > 7 && now <= 11) {
            text = '上午好';
        } else if (now > 11 && now <= 14) {
            text = '午好，这就回去了？今天收获不错吧？';
